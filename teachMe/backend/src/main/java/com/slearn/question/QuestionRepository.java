...
*/
    ...
    @Query("select distinct q from Question q inner join q.questionActivities qa where qa.user.id = :userId and qa.lesson.id = :lessonId and qa.status = :ans")
    List<Question> getByUserAndLessonAndAnswer(@Param("userId") long userId, @Param("lessonId") long lessonId, @Param("ans") boolean ans);


    @Query("select distinct q from Question q inner join q.questionActivities qa where qa.user.id = :userId and qa.status = :ans")
    List<Question> getByUserAndAnswer(@Param("userId") long userId, @Param("ans") boolean ans);
